/*!
 * @file fc_text.cpp
 * @brief `fc_text.h` の実装。Windows の `MultiByteToWideChar` / `WideCharToMultiByte`。
 *
 * Windows 以外（Android / Quest の `libfroxcore.so`）では
 * `platform/portable/cp932.cpp` の表引きに落ちる。**返し方の約束は同じ**
 * ——読めないバイトは `?`、CP932 に無い字は空——なので呼び手は場合分けを持たない。
 *
 * `silq/adapter/sq_text.cpp` の写し（差は `fc_text.h` の冒頭を見よ）。
 */
...
std::string utf8_to_sjis(const std::string &utf8)
{
     ...
     * 呼び手（`fc_lang`）は「空 = 取り込まない」を選べる必要がある。
     */
...
void FcKeyDecoder::reset()
{
    this->pending_.clear();
}

void FcKeyDecoder::flush_pending(std::vector<int> &out)
{
    ...
    if (sjis.empty()) {
        ...
        std::fprintf(stderr, "[frox] dropped %u undecodable key byte(s)\n",
...
void FcKeyDecoder::feed(const std::vector<int> &in, std::vector<int> &out)
{
    ...
    /* 1 通ぶん読み切った。**閉じ切れる塊だけ流す**（半端な尻尾は次の `feed()` の頭へ）。 */
    if (!this->pending_.empty()) {
        const std::string sjis = utf8_to_sjis(this->pending_);
        if (!sjis.empty()) {
            this->flush_pending(out);
