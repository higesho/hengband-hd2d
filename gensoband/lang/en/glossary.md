# 幻想蛮怒 英語化 用語表（版 0.9 / 2026-08-24。**承認待ちの原案**）

定義の置き場の親: （E0.5）。
**この表が承認されるまで本文の訳へ入らない**（同 §2 制約 7）。
承認後は本表が固有名詞の正となり、`lang/en/edit/*.en.txt`（E1）と
`messages.en.txt` の訳語はここに従う。

## 0. 方針

| # | 方針 |
|---|---|
| 1 | **名前の並びは公式の対外表記に従う。** 洋順（名・姓: Reimu Hakurei）が基本。**「の」を含む名**（Fujiwara no Mokou・Mononobe no Futo・Soga no Tojiko・Hata no Kokoro・Toyosatomimi no Miko・Hieda no Akyuu・Watatsuki no Yorihime/Toyohime）と**中華姓**（Hong Meiling・Son Biten）は公式どおり原順のまま |
...
| 6 | HUD の桁が決まっている札（左の状態列 20 桁など）は**短い形**を使う。短形の個別決定は E5 の幅検査時に `messages.en.txt` の `W:` と一緒に行う（本表は正式な長い形を持つ） |
...
## 3. 町（`w_info.txt` の 15＋基本 1。既存 `W:E:` は変愚の残骸で全部使わない）
...
## 4. ダンジョン（`d_info.txt` 17 本。ふざけたタグは全部直す）
...
欄: 番号 / 日本語 / **英語名**（― = いまの E: のままで良い）/ 二つ名の英訳 / 出どころ。
同名の別番号（段階ボス）は 1 行にまとめた。ID 1088 以前は変愚在来（方針 3。触らない）。
...
**変えない（現 E: のままで正しい）**: 1089 Rumia・1090 Cirno・1092-1112 の洋名勢・
1143/1144/1158 の「の」持ち・1145 Wakasagihime・1146 Sekibanki・1159 Medicine・
1207/1254 UMA・1257 Hakusen・1261 Ninja Slayer・1273-1281 の月勢・1299 Reisen・1348 Laika。
また **1088 三上山の大ムカデ（Great Centipede of Mt. Mikamisan）は良い英語なのでそのまま**。
...
店主・建物の主人の全数は E1 の `town.en.txt` で扱い、本表の人物に一致する者は本表に従う。
それ以外（幻想蛮怒オリジナルの店主）はローマ字化（§0 方針 4）。

## 7. ランダムユニークの名簿（`ru_name*` 3 本）

**ローマ字化**（2026-08-24 に決めた。設計 §13 問 4）。名の断片（接頭語・女性名・
妖怪名）をヘボン式で音写し、英語名の新造はしない。作成は E4 の `build_en_file.py`。
