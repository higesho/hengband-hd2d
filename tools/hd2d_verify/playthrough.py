#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""実際に画面とコアを起動して、起動から終了までの動作を基準と比較します。

## 目的

`golden.py` は検査モード（`--*-check`）の出力を見ますが、ゲームループ（`run()`）は通りません。
このスクリプトは実際に起動して、起動、コアとの握手、セーブの読み込み、描画、終了まで
進め、そのあいだに観測できるものを基準として保存します。

  1. 絵が表示されているか（`--shot=` で保存した BMP。色は比較しません。下記）
  2. コアとの通信（`--protocol-log=` の JSON Lines。時刻は伏せます）
  3. 画面が stderr に出した `[hd2d]` の行

## シナリオ

  title  タイトル画面まで。起動、握手、ループ、描画、終了処理を一通り通ります
  load   固定のセーブ DBG を読み込み、世界（地形、光、影）を描くところまで進めます。
         灯りの計算を通すため、時刻を 17:30 に固定します（`HD2D_FORCE_TIME`）
  pad    画面側の入力処理（`route_pad_direction` / `route_pad_press`）を通します

## 絵の色を比較しない理由

コアは別プロセスで実時間で動くので、絵を保存するフレームまでに届いている
コアからのフレーム数が実行のたびに変わります（`image_health()` にも書いています）。
画面側の時計を止めても（`HD2D_FIXED_CLOCK=1`）解消しないことを確かめています。
...
## 使い方
    ...
    python tools/hd2d_verify/playthrough.py --record   # 基準を作る
    python tools/hd2d_verify/playthrough.py --check    # 基準と比較する
...
`--shot` で終わる経路は `hd2d.cfg` を書き換えません。ただし基準は特定の `hd2d.cfg` で
作ったものなので、cfg を消して既定値に戻ると一致しなくなります。

## 見ていないところ

実時間（`SDL_Delay`、垂直同期）、実機の入力装置、音、VR の実機、ウィンドウの操作、
Android 固有の処理、そしてシナリオが通らなかった場面。
"""
...
def image_health(bmp: bytes) -> list:
    """絵が表示されているかだけを見ます。色は比較しません。

    コアは別プロセスで実時間で動き、受信は別スレッドが行います
    （`hd2d/net/core_link.cpp` の `receive_loop`）。画面は待たずに最新のフレームを
    取るだけなので、絵を保存する時点（`frames_with_terrain` で数えたループの回数）
    までにコアからいくつフレームが届いているかが、実行のたびに変わります。
    ループの回数が同じでも、そのとき描いている世界の進み具合は揃いません。

    画面側の時計を止めても（`HD2D_FIXED_CLOCK=1`）解消しません。3 回のうち 1 回はずれ、
    止めない場合も 3 回のうち 1 回は一致しました。画面側の時間による変化（埃、風、
    警戒のリング、部屋ごとの色の変化）を止めても残るということです。GPU の描画自体は
    決定的で、`replay.py` では同じ経路に固定の入力を流すと SHA-256 が完全に一致します。

    そのため、ここでは「世界が表示されたか」「真っ暗でないか」だけを見て、生きたコアと
    画面がかみ合っていることを確かめます。絵の正確さは `replay.py` が担当します。
    """
    import struct
